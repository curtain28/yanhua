...
// 移动端配置参数
const mobileConfig = {
    particleCount: 300,          // 减少粒子数量
    maxFireworks: 10,           // 减少最大烟花数量
    trailLength: 5,             // 减少拖尾长度
    particleSize: 1.5,          // 调整粒子大小
    textParticleRatio: 0.5,     // 减少文字粒子比例
    launchInterval: 400,        // 增加发射间隔
    risingSpeed: 8             // 降低上升速度
};
...
class Particle {
    constructor(x, y, color, angle, speed, life) {
        ...
        // 根据设备类型设置参数
        if(isMobileDevice()) {
            this.trailLength = mobileConfig.trailLength;
            this.size = mobileConfig.particleSize;
        } else {
            this.trailLength = 10;
            this.size = 2;
        }
...
class Firework {
    ...
    init() {
        ...
        // 修改文字粒子生成逻辑
        if (config.textParticles.enabled && 
            Math.random() < config.textParticles.probability && 
            !isMobileDevice()) {  // 添加移动设备检查
...
function randomColor() {
    ...
    const g = Math.floor(Math.random() * 255); // 随机绿色
    const b = Math.floor(Math.random() * 255); // 随机蓝色值
    return `rgb(${r},${g},${b})`; // 返回RGB色
}

// 添加新变量来跟踪用户交互
let userInteractionTimer = null;
const CLICK_COOLDOWN = 100; // 点击冷却间到100ms
...
function animate() {
    ...
    requestAnimationFrame(animate); // 请求下帧动画
}
...
function createFirework(x, y) {
    ...
    const maxFireworks = isMobile ? mobileConfig.maxFireworks : config.maxFireworks;
    ...
    if (mainFireworksCount < maxFireworks + 5) {
        ...
        const risingFirework = new RisingFirework(
            ...
            canvas.height,
            y,
            isMobile ? mobileConfig.risingSpeed : config.risingSpeed * 1.2
        );
        ...
        setTimeout(() => {
            if (!autoLaunchEnabled) {
                ...
                const interval = isMobile ? mobileConfig.launchInterval : config.launchInterval;
                autoLaunchInterval = setInterval(autoLaunch, interval);
            }
        }, 1000);
    }
}

// 修改 autoLaunch 函数，确保自动发射的烟花也在安全区域内
function autoLaunch() {
    ...
    if (mainFireworksCount < maxFireworks) {
        ...
        for (let i = 0; i < fireworksToLaunch; i++) {
            ...
            if(isMobile) {
                // 移动端水平方向保持10%的边距
                const margin = canvas.width * 0.1;
                xPosition = margin + Math.random() * (canvas.width - margin * 2);
            } else {
            ...
            const risingFirework = new RisingFirework(
                ...
                canvas.height,
                targetYPosition,
                isMobile ? mobileConfig.risingSpeed : config.risingSpeed
            );
            risingFireworks.push(risingFirework);
        }
    }
...
launchIntervalInput.addEventListener('input', (e) => {
    ...
    autoLaunchInterval = setInterval(autoLaunch, config.launchInterval); // 新自动发射间隔
});
    ...
    for (let i = 0; i < pixels.length; i += 4) {
        if (pixels[i + 3] > 128) {
            ...
            if (px % spacing === 0 && py % spacing === 0) {
                ...
                const particle = new Particle(
                    x + Math.cos(randomAngle) * randomRadius,  // 更大范的随机起始位置
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 如果需要其他 DOM 加载完成后的初始化代码，可以放在这里
    // 比如初始化声音控件
    const soundOptionsContainer = document.querySelector('.sound-options');
    if (soundOptionsContainer) {
        soundOptionsContainer.innerHTML = soundOptionsHtml;
    }
